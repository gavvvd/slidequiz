...
class _QuestionFormScreenState extends State<QuestionFormScreen> {
  ...
  void _initializeForm() {
    if (widget.question != null) {
      ...
      if (_selectedType == Question.typeMultipleChoice) {
        ...
        _choiceControllers = choices.map((c) => TextEditingController(text: c.text)).toList();
        ...
        try {
           final correctIndex = choices.indexWhere((c) => c.text == q.answer || c.label == q.answer);
           if (correctIndex != -1) {
        ...
        if (q.acceptedAnswers != null && q.acceptedAnswers!.isNotEmpty) {
          _answerListControllers = q.acceptedAnswers!.map((a) => TextEditingController(text: a)).toList();
        } else {
        ...
        if (q.acceptedAnswers != null && q.acceptedAnswers!.isNotEmpty) {
          ...
          _answerListControllers = q.answer.split(',').map((a) => TextEditingController(text: a.trim())).toList();
        }
      }
    } else {
  ...
  void _initTypeDefaults() {
    if (_selectedType == Question.typeMultipleChoice) {
      ...
      _answerListControllers = List.generate(2, (_) => TextEditingController()); // Min 2
      _isOrdered = false;
    }
  }
  ...
  void dispose() {
    ...
    for (var c in _choiceControllers) c.dispose();
    for (var c in _answerListControllers) c.dispose();
    super.dispose();
  }
  ...
  void _removeChoice(int index) {
    if (_choiceControllers.length > 2) {
      setState(() {
        ...
        if (_correctChoiceIndex == index) _correctChoiceIndex = null;
        else if (_correctChoiceIndex != null && _correctChoiceIndex! > index) _correctChoiceIndex = _correctChoiceIndex! - 1;
  ...
  Future<void> _saveQuestion() async {
    ...
    if (_selectedType == Question.typeMultipleChoice) {
      final filled = _choiceControllers.where((c) => c.text.trim().isNotEmpty).length;
      if (filled < 2) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Multiple choice must have at least 2 valid options')));
        return;
      }
      if (_correctChoiceIndex == null) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select the correct answer (check a box)')));
        return;
      }
      if (_choiceControllers[_correctChoiceIndex!].text.trim().isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Selected answer cannot be empty')));
        return;
      }
      ...
      if (_isTrueSelected == null) {
         ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select True or False')));
         return;
      }
      ...
      // Filter empty
      acceptedAnswers = _answerListControllers.map((c) => c.text.trim()).where((t) => t.isNotEmpty).toList();
      if (acceptedAnswers.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please add at least one valid answer')));
        return;
      }
      ...
      if (acceptedAnswers.length < 2) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Enumeration must have at least 2 answers')));
        return;
      }
    ...
    final int? timer = _timerController.text.isNotEmpty ? int.parse(_timerController.text) : null;
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: const [
                ...
                DropdownMenuItem(value: 'True or False', child: Text('True or False')),
            ...
            TextFormField(
              ...
              maxLines: 3,
              validator: (val) => val == null || val.trim().isEmpty ? 'Enter a question' : null,
            ),
            const SizedBox(height: 24),

            // DYNAMIC CONTENT BASED ON TYPE
            if (_selectedType == Question.typeMultipleChoice) _buildMultipleChoiceUI(),
            if (_selectedType == Question.typeTrueFalse) _buildTrueFalseUI(),
            if (_selectedType == Question.typeIdentification) _buildIdentificationUI(),
            if (_selectedType == Question.typeEnumeration) _buildEnumerationUI(),
              ...
              children: [
                ...
                Expanded(
                  child: TextFormField(
                    ...
                    decoration: const InputDecoration(labelText: 'Timer (sec)', border: OutlineInputBorder(), hintText: 'Optional'),
                    keyboardType: TextInputType.number,
                  ),
                ),
              ],
  ...
  Widget _buildMultipleChoiceUI() {
    return Column(
      ...
      children: [
        const Text('Answer Choices (Select correct answer)', style: TextStyle(fontWeight: FontWeight.bold)),
            ...
            child: Row(
              children: [
                Checkbox(
                  ...
                  onChanged: (val) {
                    setState(() => _correctChoiceIndex = (val == true) ? index : null);
                  },
                ),
                Expanded(
                  child: TextFormField(
        ...
        if (_choiceControllers.length < 6)
          TextButton.icon(onPressed: _addChoice, icon: const Icon(Icons.add), label: const Text('Add Option')),
      ],
    );
  }
          ...
          children: [
            InkWell(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: _isTrueSelected == true ? Colors.green[100] : Colors.grey[100],
                  border: Border.all(color: _isTrueSelected == true ? Colors.green : Colors.grey),
                ...
                child: Row(
                  children: [
                    ...
                    const Text('True', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
            ),
            InkWell(
              ...
              child: Container(
                padding: const EdgeInsets.symmetric(horizontal: 32, vertical: 16),
                decoration: BoxDecoration(
                  color: _isTrueSelected == false ? Colors.red[100] : Colors.grey[100],
                  border: Border.all(color: _isTrueSelected == false ? Colors.red : Colors.grey),
                  borderRadius: BorderRadius.circular(8),
                ),
                child: Row(
                  children: [
                    ...
                    const Text('False', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
            ),
  ...
  Widget _buildIdentificationUI() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Accepted Answers (Min 1)', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
            ...
            child: Row(
              children: [
                ...
                if (_answerListControllers.length > 1)
                  IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: () => _removeAnswerInput(index)),
              ],
            ),
          );
        }),
        TextButton.icon(onPressed: _addAnswerInput, icon: const Icon(Icons.add), label: const Text('Add Alternative Answer')),
      ],
    );
  }
    ...
    return Column(
      ...
      children: [
        Row(
          ...
          children: [
            const Text('Answers (Min 2)', style: TextStyle(fontWeight: FontWeight.bold)),
            Row(
              children: [
                const Text('In Order?'),
                Switch(value: _isOrdered, onChanged: (v) => setState(() => _isOrdered = v)),
              ],
            ),
          ],
        ),
        ...
        TextButton.icon(onPressed: _addAnswerInput, icon: const Icon(Icons.add), label: const Text('Add Answer')),
      ],
    );
  }
}
